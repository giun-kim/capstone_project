        ...
        methods : {
            ...
            selected(day){
                if(this.categori == "배달 완료"){   //날짜를 사용자가 바꿧을 때 해당 값으로 요청하기 위해
                    ...
                    Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/complete/'+this.mode+'/'+this.term+'/'+this.getFormDate(day.date)) //배달 완료 건 수 첫 로딩
                    .then((response) => {
                    ...
                    if(this.term == 'day'){
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waitcancel/'+this.mode+'/'+this.term+'/'+this.getFormDate(day.date))
                        .then((response) => {
                    ...
                    if(this.term == 'day'){
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waittimeavg/'+this.term+'/'+this.getFormDate(day.date))
                        .then((response) => {
            ...
            loaded(){       //맨 첨에 로딩 될 때, 각종 카테고리, 모드, 기간 클릭했을 때 나오도록.
                if(this.categori == "배달 완료"){   //배달 완료 : 일간, 주간, 월간 버튼 눌렀을 시 로딩 되는 것.
                    if(this.term=='day'){
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/complete/'+this.mode+'/'+this.term+'/'+this.getFormDate(this.date)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.complete_date = response.data.date_info.reverse();
                            this.complete_number = response.data.statis_info.reverse();
                            this.datacollection = {
                        ...
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/complete/'+this.mode+'/'+this.term+'/'+this.getFormDate(this.attributes[0].dates.start)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.complete_date = response.data.date_info.reverse();
                            this.complete_number = response.data.statis_info.reverse();
                            this.datacollection = {
                        ...
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/complete/'+this.mode+'/'+this.term+'/'+this.getFormDate(new Date(parse_month[0], parse_month[1]-1, 1))) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.complete_date = response.data.date_info.reverse();
                            this.complete_number = response.data.statis_info.reverse();
                            this.datacollection = {
                    ...
                    if(this.term=='day'){
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waitcancel/'+this.mode+'/'+this.term+'/'+this.getFormDate(this.date)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.waiting_status_date = response.data.date_info.reverse();
                            this.waiting_complete = response.data.wait_count.reverse();
                            this.waiting_cancel = response.data.wait_cancel.reverse();
                        ...
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waitcancel/'+this.mode+'/'+this.term+'/'+this.getFormDate(this.attributes[0].dates.start)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.waiting_status_date = response.data.date_info.reverse();
                            this.waiting_complete = response.data.wait_count.reverse();
                            this.waiting_cancel = response.data.wait_cancel.reverse();
                        ...
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waitcancel/'+this.mode+'/'+this.term+'/'+this.getFormDate(new Date(parse_month[0], parse_month[1]-1, 1))) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.waiting_status_date = response.data.date_info.reverse();
                            this.waiting_complete = response.data.wait_count.reverse();
                            this.waiting_cancel = response.data.wait_cancel.reverse();
                    ...
                    if(this.term=='day'){
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waittimeavg/'+this.term+'/'+this.getFormDate(this.date)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                        ...
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waittimeavg/'+this.term+'/'+this.getFormDate(this.attributes[0].dates.start)) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.wait_time_avg_date = response.data.date_info.reverse();
                            this.wait_time_avg = response.data.statis_info.reverse();
                            this.datacollection = {
                        ...
                        var parse_month = this.selected_month.split('/');
                        console.log(new Date(parse_month[0], parse_month[1]-1, 1))
                        Axios.get('http://64f5912e.ngrok.io/api/dlvy/statistics/waittimeavg/'+this.term+'/'+this.getFormDate(new Date(parse_month[0], parse_month[1]-1, 1))) //배달 완료 건 수 첫 로딩
                        .then((response) => {
                            this.wait_time_avg_date = response.data.date_info.reverse();
                            this.wait_time_avg = response.data.statis_info.reverse();
                            this.datacollection = {
